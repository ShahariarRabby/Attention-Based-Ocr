...
def process_args(args, defaults):
    ...
    parser_predict.add_argument('--dirpath',dest="dirpath",type=str,default='~/Documents/IITB/tmp/',help="Path of Directory to test.")
...
def main(args=None):
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        if parameters.phase == 'train':
            ...
            output_file_txt = open(output_file_name+'.txt',"w")
            ...
            #for line in os.listdir(dirpath):
            ...
            #        continue
            #    text, probability = model.predict(img_file_data)
            #    logging.info(filename + '  '+ text)
            #    output_file.write(filename + '  '+ text)
